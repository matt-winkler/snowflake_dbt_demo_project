{% macro create_primary_key(table_relation, column_names) %}
  {{return(adapter.dispatch('create_primary_key')(table_relation, column_names))}}
{% endmacro %}

{%- macro snowflake__create_primary_key(table_relation, column_names, quote_columns=false, constraint_name=none, lookup_cache=none) -%}
{%- set constraint_name = (constraint_name or table_relation.split('.')[2] ~ "_" ~ column_names|join('_') ~ "_PK") | upper -%}
