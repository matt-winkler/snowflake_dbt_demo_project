{% materialization table, adapter='snowflake', supported_languages=['sql', 'python']%}
  ...
  {%- set snowflake_warehouse = config.get('snowflake_warehouse') -%}
  ...
  {%- set current_warehouse_size = get_current_warehouse_size(snowflake_warehouse) -%} 
  {%- set optimal_warehouse_size = set_warehouse_size_from_explain_plan(compiled_code) -%}
  ...
  {% set alter_warehouse_sql %}
     alter warehouse {{snowflake_warehouse}} set warehouse_size = '{{optimal_warehouse_size}}'; 
  {% endset %}
  
  {{ log("current warehouse size is " ~ current_warehouse_size ~ ", setting to " ~ optimal_warehouse_size) }}
  {% do run_query(alter_warehouse_sql) %}
